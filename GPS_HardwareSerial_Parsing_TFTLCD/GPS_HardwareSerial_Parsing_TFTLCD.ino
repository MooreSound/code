...
void setup()
{
  ...
  // also spit it out
  Serial.begin(115200); Serial.println(F("TFT LCD test"));
  ...
  tft.println("GPS Test");

}

void loop() // run over and over again
{
  int brightnessValue = analogRead(lightSensePin);
int brightness = map(brightnessValue, 0, 1023, 50, 255);
...
if (GPS.hour < 10) { Serial.print('0'); }
    ...
    if (GPS.fix) {
int speed_mph = GPS.speed * 1.15078;

 tft.fillRect(0, 15, 300, 50, BLACK);
  tft.setCursor(0, 0);
  tft.setTextColor(WHITE);  tft.setTextSize(1);
  tft.println("GPS Test");
  tft.setCursor(0, 15);
  if(speed_mph >= 80){tft.setTextColor(RED);}
  else if(speed_mph >= 60){tft.setTextColor(YELLOW);}
  else {tft.setTextColor(GREEN);}
   tft.setTextSize(5);
  tft.print(speed_mph); tft.println(" MPH");


tft.fillRect(0, 80, 300, 200, BLACK);
  tft.setCursor(0, 80);
  tft.setTextColor(WHITE);  tft.setTextSize(2);
  tft.print("LAT: ");
...
tft.print("Connected Satellites: "); 
